...
-(NSURLSessionTask*) addContactsToGroupWithGroupId: (NSString*) groupId
    ...
    if (updateGroupContacts == nil) {
        NSParameterAssert(updateGroupContacts);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"updateGroupContacts"] };
            NSError* error = [NSError errorWithDomain:kOAIGroupControllerApiErrorDomain code:kOAIGroupControllerApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
