...
-(NSURLSessionTask*) createTransformer1WithAITranformCreateOptions: (OAIAITranformCreateOptions*) aITranformCreateOptions
    ...
    if (aITranformCreateOptions == nil) {
        NSParameterAssert(aITranformCreateOptions);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"aITranformCreateOptions"] };
            NSError* error = [NSError errorWithDomain:kOAIAIControllerApiErrorDomain code:kOAIAIControllerApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableDictionary *formParams = [[NSMutableDictionary alloc] init];
    NSMutableDictionary *localVarFiles = [[NSMutableDictionary alloc] init];
    bodyParam = aITranformCreateOptions;
