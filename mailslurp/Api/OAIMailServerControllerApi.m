...
-(NSURLSessionTask*) getDnsLookupsWithDNSLookupsOptions: (OAIDNSLookupsOptions*) dNSLookupsOptions
    ...
    if (dNSLookupsOptions == nil) {
        NSParameterAssert(dNSLookupsOptions);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"dNSLookupsOptions"] };
            NSError* error = [NSError errorWithDomain:kOAIMailServerControllerApiErrorDomain code:kOAIMailServerControllerApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/mail-server/describe/dns-lookups"];
