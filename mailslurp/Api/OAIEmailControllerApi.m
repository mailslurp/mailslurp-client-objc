...
-(NSURLSessionTask*) getEmailHTMLQueryWithEmailId: (NSString*) emailId
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/emails/{emailId}/html-query"];
    ...
    if (htmlSelector != nil) {
        pathParams[@"htmlSelector"] = htmlSelector;
    }
...
-(NSURLSessionTask*) getEmailTextLinesWithDecodeHtmlEntities: (NSNumber*) decodeHtmlEntities
    ...
    if (decodeHtmlEntities == nil) {
        NSParameterAssert(decodeHtmlEntities);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"decodeHtmlEntities"] };
            NSError* error = [NSError errorWithDomain:kOAIEmailControllerApiErrorDomain code:kOAIEmailControllerApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/emails/{emailId}/text"];

    NSMutableDictionary *pathParams = [[NSMutableDictionary alloc] init];
    if (decodeHtmlEntities != nil) {
        pathParams[@"decodeHtmlEntities"] = decodeHtmlEntities;
    }
    if (emailId != nil) {
        pathParams[@"emailId"] = emailId;
    }
