...
#import "OAIAliasVerificationResult.h"
...
///
///  @returns OAIAliasVerificationResult*
///
-(NSURLSessionTask*) getAliasVerificationWithAliasId: (NSString*) aliasId
    ...
    if (verificationToken == nil) {
        NSParameterAssert(verificationToken);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"verificationToken"] };
            NSError* error = [NSError errorWithDomain:kOAIAliasControllerApiErrorDomain code:kOAIAliasControllerApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/aliases/{aliasId}/verification"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"OAIAliasVerificationResult*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIAliasVerificationResult*)data, error);
                                }
                            }];
}
