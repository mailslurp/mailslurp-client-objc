...
# **waitForEmailCount**
```objc
-(NSURLSessionTask*) waitForEmailCountWithBefore: (NSDate*) before
...
// Wait for and return count number of emails. Hold connection until inbox count matches expected or timeout occurs
[apiInstance waitForEmailCountWithBefore:before
              count:count
              delay:delay
              inboxId:inboxId
              since:since
...
# **waitForLatestEmail**
```objc
-(NSURLSessionTask*) waitForLatestEmailWithBefore: (NSDate*) before
    delay: (NSNumber*) delay
    inboxId: (NSString*) inboxId
    since: (NSDate*) since
    sort: (NSString*) sort
...
// Fetch inbox's latest email or if empty wait for an email to arrive
[apiInstance waitForLatestEmailWithBefore:before
              delay:delay
              inboxId:inboxId
              since:since
              sort:sort
...
# **waitForNthEmail**
```objc
-(NSURLSessionTask*) waitForNthEmailWithBefore: (NSDate*) before
    delay: (NSNumber*) delay
    inboxId: (NSString*) inboxId
...
// Wait for or fetch the email with a given index in the inbox specified. If index doesn't exist waits for it to exist or timeout to occur.
[apiInstance waitForNthEmailWithBefore:before
              delay:delay
              inboxId:inboxId
              index:index
              since:since
