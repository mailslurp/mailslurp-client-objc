...
[**replyToEmail1**](OAIEmailControllerApi.md#replytoemail1) | **PUT** /emails/{emailId} | Reply to an email
...
# **replyToEmail1**
```objc
-(NSURLSessionTask*) replyToEmail1WithEmailId: (NSString*) emailId
...
// Reply to an email
[apiInstance replyToEmail1WithEmailId:emailId
          ...
          completionHandler: ^(OAISentEmailDto* output, NSError* error) {
                        ...
                        if (error) {
                            NSLog(@"Error calling OAIEmailControllerApi->replyToEmail1: %@", error);
                        }
                    }];
```
