...
[**getAliasVerification**](OAIAliasControllerApi.md#getaliasverification) | **GET** /aliases/{aliasId}/verification | Get validation result from alias verification
...
```objc
-(NSURLSessionTask*) getAliasVerificationWithAliasId: (NSString*) aliasId
    emailAddress: (NSString*) emailAddress
    verificationToken: (NSString*) verificationToken
        completionHandler: (void (^)(OAIAliasVerificationResult* output, NSError* error)) handler;
```
...
NSString* verificationToken = @"verificationToken_example"; // verificationToken

OAIAliasControllerApi*apiInstance = [[OAIAliasControllerApi alloc] init];

// Get validation result from alias verification
[apiInstance getAliasVerificationWithAliasId:aliasId
              emailAddress:emailAddress
              verificationToken:verificationToken
          completionHandler: ^(OAIAliasVerificationResult* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling OAIAliasControllerApi->getAliasVerification: %@", error);
                        }
                    }];
```
...
[**OAIAliasVerificationResult***](OAIAliasVerificationResult.md)
